...
export default function Ingest() {
  ...
  const execute = async () => {
    ...
    } catch (e: any) { setStatus({ ok: false, msg: e.response?.data?.detail || "Failed" }); }
    setLoading(false);
  };
  ...
  return (
    <PageTransition>
      <div className="container">
        <FadeIn>
          <div className="toolbar"><span className="toolbar-title">Data Transfer</span></div>
        </FadeIn>
        ...
        {table && (
          <ScaleIn>
            <div className="panel">
              ...
              <div className="panel-body">
                <motion.div className="file-drop" onClick={() => fileRef.current?.click()}
          ...
          <FadeIn>
            <div className="panel">
              ...
              <div className="panel-body">
                ...
                {csvHeaders.map((h, i) => (
                  <motion.div className="mapper-row" key={h}
        ...
        {mappedCount > 0 && (
          <FadeIn>
            <div className="ai-panel">
              ...
              <motion.button type="button" className="btn btn-sm" onClick={analyze} disabled={aiLoading}
        ...
        {mappedCount > 0 && (
          <FadeIn>
            <div style={{ marginTop: 16, display: "flex", gap: 12, alignItems: "center" }}>
              <motion.button type="button" className="btn btn-primary" onClick={execute} disabled={loading}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                {loading ? "Executing..." : `▶ Execute ${operation}`}
              </motion.button>
              {status && (
                <motion.span className={`badge ${status.ok ? "badge-success" : "badge-failed"}`}
