...
class JobSlaService:
    ...
    def get_weekly_trend(self, job_id: int) -> list[dict]:
        """Get weekly aggregated trend data for the last 90 days."""
        try:
            rows = self.connector.execute_query(WEEKLY_TREND, {"job_id": job_id})
            return [
    ...
    def get_monthly_trend(self, job_id: int) -> list[dict]:
        """Get monthly aggregated trend data for the last 90 days."""
        try:
            rows = self.connector.execute_query(MONTHLY_TREND, {"job_id": job_id})
            return [
    ...
    def get_trend_insights(self, job_id: int) -> Optional[dict]:
        ...
        try:
            ...
            # Current window covers today - 89 days → today
            from datetime import date, timedelta  # local import to keep module light
